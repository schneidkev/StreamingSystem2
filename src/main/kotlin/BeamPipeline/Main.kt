...
/*fun buildPipeline(pipeline: Pipeline) = pipeline.apply(
        ...
        .apply(ParDo.of(object: DoFn<String, KV<Int, java.util.List<Double>>>(){
            ...
            fun processElement(@Element element: String, out: OutputReceiver<KV<Int, java.util.List<Double>>>){
                ...
                out.output(KV.of(id as Int, values as java.util.List<Double>))
            }
        }))
        .apply(GroupByKey.create())
    .apply(ParDo.of(object: DoFn<KV<Int, Iterable<java.util.List<Double>>>, KV<Int,Iterable<Double>>>(){
        @ProcessElement
        fun processElement(@Element element: KV<Int, Iterable<java.util.List<Double>>>, out: OutputReceiver<KV<Int,Iterable<Double>>>){
            val id = element.key
            val values = element.value.flatMap { it.map { it } }
            out.output(KV.of(id, values))
        }
    }))*/
...
fun buildPipeline(pipeline: Pipeline) {
    pipeline.apply(
        KafkaIO.read<String, String>()
            ...
            .withValueDeserializer(StringDeserializer::class.java)
            .withConsumerConfigUpdates(mapOf("auto.offset.reset" to "earliest"))
    ).apply(Filter.by(SerializableFunction<KafkaRecord<String, String>, Boolean> { input ->
        input.kv.value.split(";").let { parts ->
            parts.size > 2 && parts.drop(2).all { it.isNotEmpty() && it.toDouble() > 0 }
        }
    })).apply(ParDo.of(object : DoFn<KafkaRecord<String, String>, String>() {
        @ProcessElement
        fun processElement(@Element element: KafkaRecord<String, String>, out: OutputReceiver<String>) {
    ...
    })).apply(WithTimestamps.of(SerializableFunction<String, Instant> { data ->
        Instant.parse(data.split(";")[0])
    }).withAllowedTimestampSkew(Duration(Long.MAX_VALUE)))
        .apply(Window.into(FixedWindows.of(Duration.standardSeconds(30))))
        .apply(ParDo.of(object : DoFn<String, KV<Int, Double>>() {
            @ProcessElement
            fun processElement(@Element element: String, out: OutputReceiver<KV<Int, Double>>) {
                val parts = element.split(";")
                val id = parts[1].toInt()
                val values = parts.drop(2).map { it.toDouble() }
                values.forEach{
                    out.output(KV.of(id, it))
                }
            }
        }))
        .apply(GroupByKey.create())
...
fun main(args: Array<String>) {
    ...
    processElementMethod.schemaElementParameters?.forEach { println(it) }
